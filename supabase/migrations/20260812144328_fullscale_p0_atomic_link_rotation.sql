-- P0-4: atomic respondent-link rotation.
-- Revoking the old token and issuing the new one must be one transaction, so
-- two links can never be simultaneously active for the same group/mode.
...
-- Clean up the pre-existing duplicates created by the non-atomic rotation.
-- Keep the link that has actually been used (or the oldest if none has),
-- so any token already distributed keeps working.
with ranked as (
  select id,
         row_number() over (
           partition by campaign_id, group_id
           order by (coalesce(used_count,0) > 0) desc, created_at asc
         ) as rn
  from fs_links
  where active and mode = 'group'
)
update fs_links l
...
create unique index if not exists fs_links_one_active_group_link
  on fs_links (campaign_id, group_id)
  where active and mode = 'group';
...
as $function$
declare
  v_org uuid; v_new_id uuid; v_new_token text; v_prev uuid; v_count int;
begin
  ...
  if not fs_role_in(v_org, array['owner','manager']) then
    raise exception 'Only owners and managers can rotate respondent links';
  end if;
  if p_mode not in ('group','unique') then raise exception 'Invalid link mode %', p_mode; end if;

  -- Serialise concurrent rotations for this campaign/group.
